...
<script>
    mapboxgl.accessToken = 'pk.eyJ1IjoiY2FuZGljZWhhbGxzZXR0IiwiYSI6ImNsczRydmJrbTE4cDYya3BpeWVwanRkYW8ifQ.WJrp7UCOMY1KqY5UjKCwKA';

    var map = new mapboxgl.Map({
        container: 'map',
        style: 'mapbox://styles/mapbox/streets-v11',
        center: [-95, 37], // Centered on the United States
        zoom: 4, // Adjust the zoom level as needed
        maxBounds: [
            [-179, 18], // Southwest bound
            [-66, 71]   // Northeast bound
        ]
    });

    // Add a layer for the United States
    map.on('load', function () {
        map.addSource('admin-0', {
            'type': 'vector',
            'url': 'mapbox://mapbox.boundaries-admin-0-v4'
        });
        ...
        map.addLayer({
            ...
            'type': 'fill',
            'source': 'admin-0',
            'source-layer': 'boundaries_admin_0_v3',
            'filter': ['id', 'iso_3166_1_alpha_2', '==', 'US'],
            'layout': {},
            'paint': {
                'fill-color': '#088',
                'fill-opacity': 0.8
        ...
        // Highlight on hover
        map.on('mousemove', 'usa-layer', function (e) {
            map.getCanvas().style.cursor = 'pointer';
            map.setFeatureState(
                { source: 'admin-0', sourceLayer: 'boundaries_admin_0_v3', id: e.features[0].id },
                { hover: true }
            );
        });

        // Remove highlight when not hovering
        map.on('mouseleave', 'usa-layer', function () {
            map.getCanvas().style.cursor = '';
            map.setFeatureState(
                { source: 'admin-0', sourceLayer: 'boundaries_admin_0_v3', id: null },
                { hover: false }
            );
        });
        ...
        map.on('click', 'usa-layer', function (e) {
            ...
            map.fitBounds(e.features[0].geometry.coordinates[0], { padding: 20 }); // Adjust padding as needed
        });
    });
