@extends('layouts.backend')
@section('content')
<style>
  .error-field {
    border-color: red;
  }

  .text-muted {
    font-size: 12px;
    color: #6c757d;
  }

  .view-detail-row {
    margin-bottom: 10px;
    border-bottom: 1px solid #eee;
    padding-bottom: 5px;
  }

  .view-detail-row:last-child {
    border-bottom: none;
  }
</style>

<div class="content-main dental-office-content">
  <h3>Dental Offices</h3>
  <div class="">
    <div class="dental-office-parent">
      <div class="search-main search-employee">
        <form action="{{ route('dental_offices') }}" method="GET" style="display:flex; align-items:center; width:100%;">
          <input type="search" name="search" value="{{ request('search') }}" placeholder="Search office, region, area..." class="dental-office-search">
          <img src="../images/search.svg" alt="" onclick="this.closest('form').submit()" style="cursor:pointer;">
        </form>
      </div>

      @if(!Auth::user()->hasRole('SalesRepresentative'))
      <div class="add-dental-button">
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addDentalOfficeModal">Add Dental Office</button>
      </div>
      @endif
    </div>

    <div class="dental-table-main">
      <table class="dental-office-table">
        <thead class="dental-table-header">
          <tr>
            <td class="dp-none">#</td>
            <td>Office Name</td>
            <td>Email</td>
            <td class="dp-none">Contact</td>
            <td class="dp-none">Region</td>
            <td class="dp-none">Area</td>
            <td class="dp-none">Sales Person</td>
            <td>Action</td>
          </tr>
        </thead>
        <tbody>
          @forelse($dentalOffices as $index => $office)
          <tr>
            <td class="dp-none">{{ $dentalOffices->firstItem() + $index }}</td>
            <td class="dental-office-name-img">
              <img src="../images/img.svg" alt="">
              <div>
                <h6 style="margin-bottom:0;">{{ $office->name ?? 'N/A' }}</h6>
                @if($office->dr_name) <small class="text-muted">{{ $office->dr_name }}</small> @endif
              </div>
            </td>
            <td class="email-button">
              <a href="mailto:{{ $office->email }}"><button><img src="../images/email.svg" alt="">{{ Str::limit($office->email, 20) }}</button></a>
            </td>
            <td class="dp-none"><span>{{ $office->phone ?? '-' }}</span></td>

            <td class="dp-none"><span>{{ $office->region->name ?? '-' }}</span></td>
            <td class="dp-none"><span>{{ $office->state->name ?? '-' }}</span></td>
            <td class="dp-none"><span>{{ $office->salesRep->name ?? 'Unassigned' }}</span></td>

            <td class="action-icons">
              <div class="d-flex align-items-center">
                <svg data-toggle="modal" data-target="#viewModal{{ $office->id }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:20px; height:20px; margin-right:5px; cursor:pointer;">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                </svg>

                @if(!Auth::user()->hasRole('SalesRepresentative'))
                <img style="background: #e0e0e0; padding: 5px; border-radius: 4px; cursor:pointer; margin-right:5px;" src="../images/pencil.svg" alt="Edit" title="Edit" onclick="editDentalOffice({{ $office->id }})">

                <a href="{{ route('dental_offices.delete', $office->id) }}" onclick="return confirm('Delete this office?')">
                  <img src="../images/trash.svg" style="background: #e0e0e0; padding: 5px; border-radius: 4px;" alt="Delete" title="Delete">
                                </a>
                @endif
              </div>
            </td>
          </tr>
          @empty
          <tr>
            <td colspan="8" class="text-center">No dental offices found</td>
          </tr>
          @endforelse
        </tbody>
      </table>
    </div>
    <div class="main-pagination">{{ $dentalOffices->links() }}</div>
  </div>
</div>

<div class="modal fade" id="addDentalOfficeModal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog form-office-modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Add Dental Office</h5>
        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
      </div>
      <form action="{{ route('dental_offices.store') }}" method="POST">
        @csrf
        <div class="modal-body">
          <div class="form-office-main">
            <div>
              <label>Office Name <span class="text-danger">*</span></label>
              <input type="text" name="name" required>
              <label>Doctor Name</label> <input type="text" name="dr_name">
              <label>Contact Person</label>
              <input type="text" name="contact_person">
              <label>Email</label>
              <input type="email" name="email">
              <label>Phone</label>
              <input type="text" name="phone">
              <label>Address</label> <input type="text" name="address">
              <label>Country</label>
              <input type="text" name="country" value="United States">
            </div>
            <div>
              <label>Select Location (State) <span class="text-danger">*</span></label>
              <select name="state_id" id="add_state_id" class="form-control" required>
                <option value="">Select State</option>
                @foreach($groupedStates as $regionName => $states)
                <optgroup label="{{ $regionName }}">
                  @foreach($states as $state)
                  <option value="{{ $state->id }}">{{ $state->name }}</option>
                  @endforeach
                </optgroup>
                @endforeach
              </select>

              <label class="mt-2">Assign Sales Rep <span class="text-danger">*</span></label>
              <select name="sales_rep" id="add_sales_rep" class="form-control" required>
                <option value="">Select Location First</option>
              </select>

              <label class="mt-2">Territory ID</label>
              <input type="number" name="territory">
            </div>
          </div>
        </div>
        <div class="modal-footer form-office-modal-footer">
          <button type="submit" class="btn btn-primary">Save</button>
        </div>
      </form>
    </div>
  </div>
</div>

<div class="modal fade" id="editDentalOfficeModal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog form-office-modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Edit Dental Office</h5>
        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
      </div>
      <form id="editForm" method="POST">
        @csrf
        <div class="modal-body">
          <div class="form-office-main">
            <div>
              <label>Office Name</label> <input type="text" name="name" id="edt_name" required>
              <label>Doctor Name</label> <input type="text" name="dr_name" id="edt_dr_name">
              <label>Contact Person</label> <input type="text" name="contact_person" id="edt_contact">
              <label>Email</label> <input type="email" name="email" id="edt_email">
              <label>Phone</label> <input type="text" name="phone" id="edt_phone">
              <label>Address</label> <input type="text" name="address" id="edt_address">
              <label>Country</label> <input type="text" name="country" id="edt_country">
            </div>
            <div>
              <label>Location (State)</label>
              <select name="state_id" id="edt_state_id" class="form-control">
                @foreach($groupedStates as $regionName => $states)
                <optgroup label="{{ $regionName }}">
                  @foreach($states as $state)
                  <option value="{{ $state->id }}">{{ $state->name }}</option>
                  @endforeach
                </optgroup>
                @endforeach
              </select>

              <label class="mt-2">Sales Rep</label>
              <select name="sales_rep" id="edt_sales_rep" class="form-control"></select>

              <label class="mt-2">Territory ID</label>
              <input type="number" name="territory" id="edt_territory">
            </div>
          </div>
        </div>
        <div class="modal-footer form-office-modal-footer">
          <button type="submit" class="btn btn-primary">Update</button>
        </div>
      </form>
    </div>
  </div>
</div>

@foreach($dentalOffices as $office)
<div class="modal fade" id="viewModal{{ $office->id }}" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog form-office-modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{{ $office->name }}</h5>
        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
      </div>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            <div class="view-detail-row"><strong>Doctor Name:</strong><br> {{ $office->dr_name ?? 'N/A' }}</div>
            <div class="view-detail-row"><strong>Contact Person:</strong><br> {{ $office->contact_person ?? 'N/A' }}</div>
            <div class="view-detail-row"><strong>Phone:</strong><br> {{ $office->phone ?? 'N/A' }}</div>
            <div class="view-detail-row"><strong>Email:</strong><br> {{ $office->email ?? 'N/A' }}</div>
          </div>
          <div class="col-md-6">
            <div class="view-detail-row"><strong>Address:</strong><br> {{ $office->address ?? 'N/A' }}</div>
            <div class="view-detail-row"><strong>Region:</strong><br> {{ $office->region->name ?? '-' }}</div>
            <div class="view-detail-row"><strong>Area:</strong><br> {{ $office->state->name ?? '-' }}</div>
            <div class="view-detail-row"><strong>Sales Rep:</strong><br> {{ $office->salesRep->name ?? '-' }}</div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
@endforeach

<script>
  $(document).ready(function() {

    // --- ADD MODAL LOGIC ---
    $('#add_state_id').change(function() {
      var id = $(this).val();
      $('#add_sales_rep').html('<option value="">Loading...</option>');

      if (id) {
        $.get('/get-sales-reps/' + id, function(res) {
          $('#add_sales_rep').html('<option value="">Select Rep</option>');
          $.each(res.reps, function(i, v) {
            $('#add_sales_rep').append('<option value="' + v.id + '">' + v.name + '</option>');
          });
        });
      } else {
        $('#add_sales_rep').html('<option value="">Select Location First</option>');
      }
    });

    // --- EDIT MODAL LOGIC ---
    $('#edt_state_id').change(function() {
      var id = $(this).val();
      $.get('/get-sales-reps/' + id, function(res) {
        $('#edt_sales_rep').html('<option value="">Select Rep</option>');
        $.each(res.reps, function(i, v) {
          $('#edt_sales_rep').append('<option value="' + v.id + '">' + v.name + '</option>');
        });
      });
    });

    // --- UPDATE FORM SUBMIT ---
    $('#editForm').submit(function(e) {
      e.preventDefault();
      var url = $(this).attr('action');
      $.post(url, $(this).serialize(), function(response) {
        location.reload();
      }).fail(function() {
        alert('Error updating dental office.');
      });
    });
  });

  // --- OPEN EDIT MODAL & PREFILL ---
  function editDentalOffice(id) {
    $.get('/dental_offices/' + id + '/edit', function(response) {
      var office = response.office;

      // 1. Fill Fields
      $('#edt_name').val(office.name);
      $('#edt_dr_name').val(office.dr_name);
      $('#edt_contact').val(office.contact_person);
      $('#edt_email').val(office.email);
      $('#edt_phone').val(office.phone);
      $('#edt_address').val(office.address);
      $('#edt_country').val(office.country);
      $('#edt_territory').val(office.territory_id);

      // 2. Set Action URL
      $('#editForm').attr('action', '/dental_offices/update/' + id);

      // 3. Set Location (State)
      $('#edt_state_id').val(office.state_id);

      // 4. Populate Reps based on state & Select Current
      $('#edt_sales_rep').empty();
      $('#edt_sales_rep').append('<option value="">Select Rep</option>');
      $.each(response.valid_reps, function(i, rep) {
        var selected = (rep.id == office.sales_rep_id) ? 'selected' : '';
        $('#edt_sales_rep').append('<option value="' + rep.id + '" ' + selected + '>' + rep.name + '</option>');
      });

      $('#editDentalOfficeModal').modal('show');
    });
  }
</script>
@endsection